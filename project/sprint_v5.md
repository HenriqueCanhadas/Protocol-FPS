...
| SPRINT | TEST | STATUS | RESULTS |
|--------|------|--------|---------|
| S47 · Criar `scrapers/aliexpress.py` (subclasse de `ScraperBase`) e registrar `"aliexpress"` no dict `SCRAPERS` de `main.py`; validar com o link de teste (todo:249) | `AliExpressScraper(headless=False).coletar('<link de teste>')` retorna um `DadosProduto` com nome/preço/disponibilidade corretos comparados à página real; seguir a metodologia da skill `scraper-nova-loja` (inspeção real antes de escolher seletor, teste headless e non-headless, e se possível 3 runs de CI reais) | ⬜ Todo | **Risco a avaliar antes de comprometer prazo:** AliExpress é um marketplace internacional de grande porte com histórico de anti-bot agressivo (challenges, preço variável por região/moeda) — o mesmo tipo de risco que já se confirmou real na Pichau (bloqueio de IP de datacenter) e na Shopee (parede de login). Resultado esperado: seguir a mesma ordem de extração já usada nos scrapers existentes (JSON-LD → meta → CSS → varredura de texto) e, se o CI travar de forma equivalente ao caso Shopee, registrar a mesma decisão honesta (scraper existe e funciona local, mas não coleta de datacenter) em vez de insistir indefinidamente. Validar preço/moeda em BRL (o link de teste é do domínio `.com.br`/`pt.aliexpress.com` — confirmar que o preço extraído já vem em Real, não em outra moeda) |
...
| SPRINT | TEST | STATUS | RESULTS |
|--------|------|--------|---------|
| S48 · Criar `scrapers/mocadopop.py` e registrar `"mocadopop"` em `SCRAPERS`; validar com o link de teste (todo:251) | `MocadopopScraper(headless=False).coletar('<link de teste>')` retorna nome/preço/disponibilidade corretos; seguir a metodologia da skill `scraper-nova-loja` | ⬜ Todo | **Decisão em aberto a confirmar com o usuário antes de implementar (o próprio `todo` já sinaliza a pergunta):** o usuário tem itens Mocadopop hoje monitorados por um projeto separado (`github.com/HenriqueCanhadas/Monitoramento`) e pediu para avaliar "mais possibilidades e melhorias" nesse projeto legado para rodar na estrutura atual — precisa decidir se (a) só o scraper novo é criado (sem migração de dados) ou (b) os itens/histórico desse projeto são migrados para o Supabase atual, no mesmo padrão já usado na Sprint 8 (`project/migrations/sprint8_diversos_migracao.sql`, categoria Diversos) — incluindo a mesma cautela de não sobrescrever nada por engano. Resultado esperado, uma vez decidido o escopo: scraper novo seguindo o padrão `ScraperBase`; se houver migração, um novo `project/migrations/sprint48_mocadopop_migracao.sql` rodado manualmente pelo usuário no SQL Editor, com validação ponta a ponta (contagem de itens/leituras migradas batendo com a origem, timezone BRT→UTC quando aplicável) |
